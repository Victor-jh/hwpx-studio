...
# ── KCUP 전용 상수 ──────────────────────────────────────────────
# charPr/paraPr 폴백 (cost_header 기준, 매핑 파일 로드 실패 시 사용)
_KCUP_FALLBACK_CP = {
...
def _init_kcup_mapping():
    m = load_kcup_mapping("cost")
    return m.get("charPr", dict(_KCUP_FALLBACK_CP)), m.get("paraPr", dict(_KCUP_FALLBACK_PP))
...
def main():
    ...
    # --header 옵션으로 KCUP 매핑 전환
    if args.header:
        global KCUP_CP, KCUP_PP
        m = load_kcup_mapping(args.header)
        KCUP_CP = m.get("charPr", dict(_KCUP_FALLBACK_CP))
        KCUP_PP = m.get("paraPr", dict(_KCUP_FALLBACK_PP))
