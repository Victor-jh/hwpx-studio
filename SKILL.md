...
사용자 요청을 받으면 아래 로직을 순서대로 실행한다. **3가지 핵심 경로** 중 하나로 반드시 귀결된다.

```python
# Step 1: 핵심 경로 결정
if 사용자가_hwpx_첨부:
    if 의도 == "읽기":   # 분석/구조파악/텍스트추출/블록확인
        → 읽기 경로
    elif 의도 == "편집": # 수정/바꿔줘/삭제/머리말변경/텍스트교체
        → 편집 경로
    elif 의도 == "생성": # 이 양식으로 새 문서/내용 전체 교체/데이터 교체
        → 생성 경로 (레퍼런스 모드)
    elif 의도 == "검증": # 검증/점검/유효성
        → validate.py 단독 실행
    else:
        → 읽기 경로 (기본값: 먼저 분석해서 상황 파악)
else:
    → 생성 경로 (새 문서 모드)

# Step 2: 경로별 스크립트 선택
읽기 경로:
    구조 분석이 필요하면  → read_document.py  (JSON 역변환, 블록 타입 감지)
    텍스트만 필요하면     → text_extract.py   (순수 텍스트/마크다운)

편집 경로:
    항상 edit_document.py 먼저 시도.
    edit_document.py로 불가능한 경우에만 → unpack.py → XML 직접 수정 → pack.py
    (불가능한 경우 = 새 스타일 추가, secPr 변경, 이미지 삽입 등 ZIP 구조 변경이 필요할 때)

생성 경로:
    레퍼런스 있으면 → analyze_template.py → build_hwpx.py (+ page_guard.py 필수)
    레퍼런스 없으면 → create_document.py --template {자동추론}
```

### 편집 vs 생성 — 판단 기준

"이 문서 수정해줘"라고 했을 때 편집/생성 중 뭘 골라야 하는가:

```python
if 사용자_요청이_아래에_해당:
    - 텍스트 찾아바꾸기 (find/replace)
    - 특정 블록 삭제/수정
    - 머리말·꼬리말 변경
    - 블록 순서 변경
    → 편집 경로 (edit_document.py)
    ...
    → 편집 경로 먼저 시도. 실패하거나 한계가 보이면 생성 경로로 전환.
```

---

## [생성] 기본 동작 모드: 첨부 HWPX → 구조 복원 → 요청 반영 재작성

사용자가 `.hwpx`를 첨부하고 **"이 양식으로 새 문서/내용만 교체"를 요청한 경우** 이 경로를 탄다.
상세 명령은 하단 **[[생성] 레퍼런스 기반 문서 생성](#생성-레퍼런스-기반-문서-생성-analyze--build--첨부-hwpx--새-내용-작성-시)** 섹션 참조.

**요약 흐름**: `analyze_template.py` → 구조 복원 → `build_hwpx.py` → `validate.py` → `page_guard.py`
...
상세 CLI 명령은 하단 **[생성] 레퍼런스 기반 문서 생성** 섹션의 "사용법" 참조.
...
## [생성] XML-first 문서 생성 (고급: 레퍼런스 없이 XML 직접 작성)
...
# 빈 문서 (base 템플릿)
python3 "$SKILL_DIR/src/hwpx_studio/build_hwpx.py" --output result.hwpx

# 템플릿 사용
python3 "$SKILL_DIR/src/hwpx_studio/build_hwpx.py" --template gonmun --output result.hwpx

# 커스텀 section0.xml 오버라이드
python3 "$SKILL_DIR/src/hwpx_studio/build_hwpx.py" --template gonmun --section my_section0.xml --output result.hwpx

# header도 오버라이드
python3 "$SKILL_DIR/src/hwpx_studio/build_hwpx.py" --header my_header.xml --section my_section0.xml --output result.hwpx

# 메타데이터 설정
python3 "$SKILL_DIR/src/hwpx_studio/build_hwpx.py" --template report --section my.xml \
...
# 2. 빌드
python3 "$SKILL_DIR/src/hwpx_studio/build_hwpx.py" --section "$SECTION" --output result.hwpx
...
## [생성] JSON→HWPX 원커맨드 파이프라인 (create_document.py) ← 기본 생성 경로
...
## [편집] 수동 XML 편집 (unpack → Edit → pack) ← edit_document.py로 불가능할 때만

```bash
source "$VENV"

# 1. HWPX → 디렉토리 (XML pretty-print)
python3 "$SKILL_DIR/src/hwpx_studio/office/unpack.py" document.hwpx ./unpacked/
...
## [읽기] HWPX → JSON 구조 읽기 (read_document.py) ← 기본 읽기 경로
...
python3 "$SKILL_DIR/src/hwpx_studio/read_document.py" document.hwpx

# Pretty-print + 파일 저장
python3 "$SKILL_DIR/src/hwpx_studio/read_document.py" document.hwpx --pretty -o output.json

# 스타일 스펙 포함 (charPr/paraPr 상세 정보를 _styles 키에 추가)
python3 "$SKILL_DIR/src/hwpx_studio/read_document.py" document.hwpx --pretty --include-styles -o output.json
```
...
## [편집] HWPX 인플레이스 편집 (edit_document.py) ← 기본 편집 경로
...
## [읽기] 텍스트 추출 (text_extract.py) ← 순수 텍스트만 필요할 때

```bash
source "$VENV"

# 순수 텍스트
python3 "$SKILL_DIR/src/hwpx_studio/text_extract.py" document.hwpx

# 테이블 포함
python3 "$SKILL_DIR/src/hwpx_studio/text_extract.py" document.hwpx --include-tables

# 마크다운 형식
python3 "$SKILL_DIR/src/hwpx_studio/text_extract.py" document.hwpx --format markdown
```
...
```bash
source "$VENV"
python3 "$SKILL_DIR/src/hwpx_studio/validate.py" document.hwpx
```
...
## [생성] 레퍼런스 기반 문서 생성 (analyze → build) ← 첨부 HWPX + 새 내용 작성 시
...
# 1. 심층 분석 (구조 청사진 출력)
python3 "$SKILL_DIR/src/hwpx_studio/analyze_template.py" reference.hwpx
...
# 6. 쪽수 드리프트 가드 (필수)
python3 "$SKILL_DIR/src/hwpx_studio/page_guard.py" \
...
| 스크립트 | 용도 |
|----------|------|
| `src/hwpx_studio/create_document.py` | **원커맨드** — JSON → section_builder → build_hwpx → validate 파이프라인 |
| `src/hwpx_studio/read_document.py` | **HWPX → JSON 역변환** — 35개 블록 타입 자동 감지, 라운드트립 지원 |
| `src/hwpx_studio/edit_document.py` | **HWPX 인플레이스 편집** — 텍스트 교체/블록 삽입·삭제·수정/머리말·꼬리말 변경 |
| `src/hwpx_studio/section_builder.py` | JSON → section0.xml 동적 생성 (35개 블록 타입, auto_spacing) |
| `src/hwpx_studio/build_hwpx.py` | 템플릿 + XML → HWPX 조립 |
| `src/hwpx_studio/analyze_template.py` | HWPX 심층 분석 (레퍼런스 기반 생성의 청사진) |
| `src/hwpx_studio/office/unpack.py` | HWPX → 디렉토리 (XML pretty-print) |
| `src/hwpx_studio/office/pack.py` | 디렉토리 → HWPX (mimetype first) |
| `src/hwpx_studio/property_registry.py` | 동적 charPr/paraPr/borderFill ID 할당 (PropertyRegistry) |
| `src/hwpx_studio/validate.py` | HWPX 파일 구조 검증 |
| `src/hwpx_studio/page_guard.py` | 레퍼런스 대비 페이지 드리프트 위험 검사 (필수 게이트) |
| `src/hwpx_studio/diff_docs.py` | 텍스트 unified diff + 구조 비교 |
| `src/hwpx_studio/text_extract.py` | HWPX 텍스트 추출 |
